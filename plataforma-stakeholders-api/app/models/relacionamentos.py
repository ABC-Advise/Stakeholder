...
class Relacionamentos(BaseModel):
    ...
    tipo_origem_id = db.Column(db.SmallInteger, db.ForeignKey(TipoEntidade.tipo_entidade_id), primary_key=True)
    entidade_destino_id = db.Column(db.Integer, primary_key=True)
    tipo_destino_id = db.Column(db.SmallInteger, db.ForeignKey(TipoEntidade.tipo_entidade_id), primary_key=True)
    tipo_relacao_id = db.Column(db.SmallInteger, db.ForeignKey(TipoRelacao.tipo_relacao_id), nullable=False)
