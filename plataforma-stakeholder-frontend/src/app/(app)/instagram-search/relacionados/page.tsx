...
export default function BuscaRelacionadosPage() {
  ...
  useEffect(() => {
    ...
    if (type && ['empresa', 'pessoa', 'advogado'].includes(type))
      setTipoRelacionado(type as any);
  }, [searchParams]);
  ...
  const handleBuscarRelacionados = async () => {
    ...
    try {
      const data = await instagramService.getEntityRelated(
        tipoRelacionado,
        idLimpo
      );
      ...
      try {
        const perfilPrincipal = await instagramService.getEntityProfile(
          tipoRelacionado,
          { identifier: idLimpo }
        );
        setPerfilPrincipal(perfilPrincipal?.entity || null);
        const insta =
          perfilPrincipal?.validated_profile?.perfil?.username ||
          perfilPrincipal?.entity?.instagram ||
          '';
        ...
        const msg =
          ...
          error?.message ||
          'Erro ao buscar perfil principal.';
        setMensagemErro(msg);
      }
      toast({
        title: 'Relacionados carregados',
        description: 'Os relacionados foram encontrados.',
      });
    } catch (error: any) {
      console.log('ERRO CAPTURADO (relacionados):', error);
      toast({
        title: 'Teste',
        description: 'Toast manual',
        variant: 'destructive',
      });
      const msg =
        error?.response?.data?.detail ||
        error?.response?.data?.message ||
        error?.message ||
        'Erro desconhecido ao buscar relacionados.';
      setMensagemErro(msg);
    } finally {
  ...
  const handleBuscarInstagramRelacionados = async () => {
    if (!instagramPrincipal) {
      setMensagemErro(
        'Instagram da entidade principal não encontrado. Cadastre manualmente ou busque.'
      );
      return;
    }
    setLoadingBuscaInstagram(true);
    try {
      const nomesRelacionados = relatedResult.related_entities
        .map((rel: any) =>
          [rel.firstname, rel.lastname].filter(Boolean).join(' ')
        )
        .filter(Boolean);
      const resultadoBusca = await instagramService.searchNamesInFollowers(
        instagramPrincipal,
        nomesRelacionados,
        0.8
      );
      const resultados = relatedResult.related_entities.map(
        (rel: any, idx: number) => {
          const nomeBusca = nomesRelacionados[idx];
          const match = resultadoBusca.matches?.find(
            (m: any) => m.name_searched === nomeBusca
          );
          return {
            ...rel,
      ...
      toast({
        title: 'Busca automática concluída',
        description: 'Resultados exibidos abaixo.',
      });
    } catch (error: any) {
      ...
      const msg =
        ...
        error?.response?.data?.message ||
        error?.message ||
        'Erro desconhecido na busca automática.';
      setMensagemErro(msg);
    } finally {
  ...
  const handleBuscaManual = async () => {
    ...
    setLoadingBuscaInstagram(true);
    try {
      const nomes = manualNames
        .split(',')
        .map(n => n.trim())
        .filter(Boolean);
      const resultadoBusca = await instagramService.searchNamesInFollowers(
        manualInstagram.trim(),
        nomes,
        0.8
      );
      setManualResults(resultadoBusca.found_names || []);
      toast({
        title: 'Busca manual concluída',
      ...
      const msg =
        ...
        error?.response?.data?.message ||
        error?.message ||
        'Erro desconhecido na busca manual.';
      setMensagemErro(msg);
    } finally {
      setLoadingBuscaInstagram(false);
    }
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      <h1 className="text-3xl font-bold mb-6">
        Encontrar caminho até{' '}
        <span className="text-blue-700">@waltaganlopes</span>
      </h1>
      {/* Passo 1: Buscar Relacionados */}
      <Card className="mb-6">
        <CardHeader>
        ...
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-2 items-end">
            <Select
              value={tipoRelacionado}
              onValueChange={v =>
                setTipoRelacionado(v as 'empresa' | 'pessoa' | 'advogado')
              }
            >
              ...
              {relatedResult.related_entities.map((rel: any, idx: number) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-600">
                    {rel.cpf && (
                      <>
                        CPF: <span className="font-mono">{rel.cpf}</span>
                      </>
                    )}
                    {rel.cnpj && (
                      <>
                        CNPJ: <span className="font-mono">{rel.cnpj}</span>
                      </>
                    )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-semibold">
                  {instagramPrincipal || 'Não informado'}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      {relatedResult && (
        <Card className="mb-6">
          <CardHeader>
            <CardTitle>
              Busca Automática de Instagram dos Relacionados
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex gap-2 mb-2">
              ...
              />
              <Button
                onClick={handleBuscarInstagramRelacionados}
                disabled={loadingBuscaInstagram}
              >
                {loadingBuscaInstagram ? (
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                ) : (
                  <Search className="mr-2 h-4 w-4" />
                )}
                      ...
                      <div className="mt-2">
                        ...
                        rel.resultado.matches.length > 0 ? (
                              ...
                              {rel.resultado.matches.map(
                                (match: any, i: number) => (
                                  ...
                                  >
                                    ...
                                    />
                                    <a
                                      href={`https://instagram.com/${match.username}`}
                                      target="_blank"
                          ...
                          <div className="bg-yellow-50 border border-yellow-200 rounded p-2 text-yellow-700">
                            Não encontrado entre os seguidores.
                          </div>
                        )}
                      </div>
                    )}
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>
            Busca Profunda entre Seguidores -Apenas Perfis Públicos-
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-2 items-end">
            ...
            />
            <Select
              value={algoritmoBusca}
              onValueChange={v => setAlgoritmoBusca(v as 'bfs' | 'dfs')}
            >
            ...
            <div className="mt-6">
              <p className="text-base font-semibold text-gray-700 mb-4">
                ...
                <span className="text-blue-700">
                  @{caminhosEncontrados.origem}
                </span>{' '}
                e{' '}
                <span className="text-blue-700">
                  @{caminhosEncontrados.alvo}
                </span>
              </p>
              <div className="space-y-4">
                {caminhosEncontrados.caminhos.map(
                  (caminho: string[], idx: number) => (
                    <div
                      key={idx}
