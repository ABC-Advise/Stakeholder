...
export function EntitiesTable({ data, onSearchIndividual, isLoading, tipo }: EntitiesTableProps) {
    const router = useRouter();
    const [sorting, setSorting] = useState<SortingState>([]);
    const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
    const [columnVisibility, setColumnVisibility] = useState<VisibilityState>({});
    const [rowSelection, setRowSelection] = useState({});
    const [internalLoading, setInternalLoading] = useState<Record<string, boolean>>({}); // Loading por linha
    ...
    const columns: ColumnDef<Entidade>[] = [
        ...
        },
        {
            ...
            filterFn: (row, id, value) => value === 'todos' || value === row.getValue(id),
        },
        {
            ...
            cell: ({ row }) => {
                ...
                const status = entidade.instagram && entidade.instagram.trim() !== "" ? "Atribuído" : "Não atribuído";
                const colorClass = status === "Atribuído" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800";
                return <span className={`px-2 py-1 rounded-full text-xs font-medium ${colorClass}`}>{status}</span>;
            },
        },
        {
            id: "identificadorPrincipal",
            header: tipo === 'empresa' ? 'CNPJ' : tipo === 'pessoa' ? 'CPF' : tipo === 'advogado' ? 'OAB' : 'CNPJ/CPF/OAB',
            cell: ({ row }) => {
                const entidade = row.original as Entidade;
    ...
    if (isLoading) {
        return (
            <div className="rounded-md border p-4">
                <div className="h-10 bg-gray-200 rounded mb-4 animate-pulse w-1/2"></div> {/* Placeholder for filters */}
    ...
    return (
        <div className="w-full">
            ...
            <div className="flex items-center py-4 gap-2 flex-wrap">
                ...
                <Select
                    value={(table.getColumn("statusInstagramManual")?.getFilterValue() as string) ?? "todos"}
                    onValueChange={(value) => table.getColumn("statusInstagramManual")?.setFilterValue(value)}
                >
                        ...
                        {table
                            ...
                            .map((column) => {
                                return (
                                    ...
                                    >
                                        {column.id === 'ultimaBusca' ? 'Última Busca' : column.id} {/* Melhorar exibição nome coluna */}
                                    </DropdownMenuCheckboxItem>
                                );
                            })}
                    </DropdownMenuContent>
