...
function renderResultados(info: any) {
    ...
    if (!output) return <span className="text-zinc-400 text-sm">Nenhuma informação disponível ainda.</span>;
                    ...
                    <ul className="list-disc pl-5">
                        {output.encontrados.map((item: any) => (
                            <li key={item.id_banco} className="mb-2">
                                ...
                                {item.username && (
                                    <span className="ml-2 text-xs text-zinc-500">({item.username})</span>
                                )}
                                <br />
                                <span className="text-xs text-zinc-700">{item.justificativa}</span>
                            </li>
                        ))}
                    </ul>
                ) : (
...
export default function FluxoValidacaoPage() {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const data: PaginatedEntities = await instagramService.searchEntities('empresa', cnpj);
            ...
            //https://n8n-automacao-production.up.railway.app/webhook-test/dd92e331-8e22-4bf5-bd02-417f336609ee
            const webhookResponse = await fetch('https://n8n-automacao-production.up.railway.app/webhook/dd92e331-8e22-4bf5-bd02-417f336609ee', {
                method: 'POST',
                headers: {
            ...
            <div className="w-full max-w-4xl flex flex-col md:flex-row gap-8">
                ...
                <div className="flex-1 flex flex-col gap-8">
                    <Card className="bg-white border border-zinc-200 shadow-md">
                        <CardHeader>
                            <CardTitle className="text-zinc-900 text-xl font-bold text-center">Iniciar Fluxo de Validação</CardTitle>
                        </CardHeader>
                    ...
                    <Card className="bg-white border border-zinc-200 shadow-md">
                        <CardHeader>
                            <CardTitle className="text-zinc-900 text-xl font-bold text-center">
                                Valida Relacionados 1<br />(Advogado e Sócios)
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="flex justify-center">
                            <Button className="bg-zinc-200 text-zinc-500 font-bold w-full cursor-not-allowed" disabled>
                                Em andamento
                            </Button>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="bg-white border border-zinc-200 shadow-md opacity-60">
                        <CardHeader>
                            <CardTitle className="text-zinc-900 text-xl font-bold text-center">Validar Empresas 2</CardTitle>
                        </CardHeader>
                        <CardContent className="flex justify-center">
                            <Button className="bg-zinc-200 text-zinc-500 font-bold w-full cursor-not-allowed" disabled>
                                Em breve
                    ...
                    <Card className="bg-white border border-zinc-200 shadow-md opacity-60">
                        <CardHeader>
                            <CardTitle className="text-zinc-900 text-xl font-bold text-center">Validar Sócios 2</CardTitle>
                        </CardHeader>
                        <CardContent className="flex justify-center">
                            <Button className="bg-zinc-200 text-zinc-500 font-bold w-full cursor-not-allowed" disabled>
                                Em breve
                            ...
                            <CardContent>
                                {isLoading ? (
                                    <div className="flex flex-col items-center justify-center gap-2 py-4">
                                        ...
                                        <span className="text-zinc-500 text-sm">Aguardando resposta do processo...</span>
                                    </div>
                                ) : webhookInfo ? (
                                    renderResultados(webhookInfo)
                                ) : (
                                    <span className="text-zinc-400 text-sm">Nenhuma informação disponível ainda.</span>
                                )}
                            </CardContent>
                        </Card>
                    </div>
