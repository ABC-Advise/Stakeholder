...
const instagramApi = axios.create({
  baseURL:
    ...
    'http://localhost:8000/api/v1', // Fallback mais explícito
});
...
export const instagramService = {
    ...
    if (tipo === 'todos') {
      ...
      try {
        const [empresasRes, pessoasRes, advogadosRes] = await Promise.all([
          instagramApi.get<PaginatedEntities>('/entities/all/empresa', {
            params: queryParams,
          }),
          instagramApi.get<PaginatedEntities>('/entities/all/pessoa', {
            params: queryParams,
          }),
          instagramApi.get<PaginatedEntities>('/entities/all/advogado', {
            params: queryParams,
          }),
        ]);
        const combinedData = [
          ...empresasRes.data.data,
          ...pessoasRes.data.data,
          ...advogadosRes.data.data,
        ];
        const total =
          empresasRes.data.total +
          pessoasRes.data.total +
          advogadosRes.data.total; // Total combinado pode não ser ideal para paginação real
      ...
      const response = await instagramApi.get<PaginatedEntities>(
        `/entities/all/${tipo}`,
        { params: queryParams }
      );
  ...
  getEntityProfile: async (
    ...
    paramsInput:
      | { entityId: string; identifier?: never }
      | { entityId?: never; identifier: string }
    ...
    try {
      const response = await instagramApi.get<ResultadoBusca>(
        `/${entityType}`,
        { params: queryParams }
      );
      return response.data;
      ...
      if (error.response && error.response.status === 404) {
        console.warn(
          `Entidade ${entityType} com params ${JSON.stringify(queryParams)} não encontrada (404). Retornando null.`
        );
        return null;
      }
      console.error(
        `Erro ao buscar perfil/candidatos para ${entityType} com params ${JSON.stringify(queryParams)}:`,
        error
      );
      throw error;
    }
   ...
   */
  findInstagramForEntity: async (
    entityType: string,
    entityId: string
  ): Promise<{ jobId: string }> => {
    const response = await instagramApi.post<{ jobId: string }>(
      `/entity/${entityType}/${entityId}/find`
    );
    return response.data;
  },
   ...
   */
  findAllInstagramForEntityType: async (
    entityType: string
  ): Promise<{ results: ResultadoBusca[] }> => {
    const response = await instagramApi.get<{ results: ResultadoBusca[] }>(
      `/entities/all/${entityType}`
    );
  ...
  validateCandidate: async (
    ...
    console.warn(
      `PROVISÓRIO: Usando endpoint POST /candidate/${candidateId}/validate não presente no Swagger.`
    );
   ...
   */
  confirmInstagramProfile: async (
    entityType: string,
    entityId: string,
    username: string
  ): Promise<void> => {
    await instagramApi.post(
      `/entity/${entityType}/${entityId}/confirm_instagram`,
      { instagram: username }
    );
  },
   ...
   */
  getAllEntitiesPaginated: async (
    pagina = 1,
    limite = 10
  ): Promise<PaginatedEntities> => {
    const response = await instagramApi.get('/entities/list/all', {
   ...
   */
  getEntitiesByTypePaginated: async (
    tipo: 'empresa' | 'pessoa' | 'advogado',
    pagina = 1,
    limite = 10
  ): Promise<PaginatedEntities> => {
    const response = await instagramApi.get(`/entities/list/${tipo}`, {
      params: { pagina, limite },
    });
    return response.data;
  },
   ...
   */
  getEntityRelated: async (
    entityType: 'empresa' | 'pessoa' | 'advogado',
    identifier: string
  ): Promise<any> => {
    const response = await instagramApi.get(`/entities/${entityType}/related`, {
      params: { identifier },
    });
    return response.data;
  },
