...
export default function ResultadoBuscaPage() {
  ...
  const carregarResultados = useCallback(async () => {
    ...
    try {
      const data = await instagramService.getEntityProfile(
        entityType as 'empresa' | 'pessoa' | 'advogado',
        { identifier: entityId }
      );
      if (data) {
        setResultado(data);
        const currentInstagram =
          data.validated_profile?.perfil.username ||
          (data.entity as Entidade).instagram;
        if (currentInstagram) {
        ...
        setResultado(null);
        setMensagemErro(
          'Entidade não encontrada. Não foi possível encontrar os dados para esta entidade.'
        );
      }
    } catch (error: any) {
      let msg =
  ...
  const handleConfirmarPerfil = async () => {
    ...
    if (!usernameToSave) {
      setMensagemErro(
        'Por favor, informe um nome de usuário do Instagram para salvar.'
      );
      return;
    }
    setIsSaving(true);
    try {
      ...
      let mensagemErro =
        ...
        error?.response?.data?.message ||
        error?.message ||
        'Não foi possível salvar o perfil para a entidade.';
    ...
    try {
      ...
      setMensagemErro(
        error?.response?.data?.detail ||
          error?.response?.data?.message ||
          error?.message ||
          'Não foi possível iniciar uma nova busca.'
      );
    } finally {
  ...
  const ValidatedProfileDisplay = ({
    profile,
    score,
  }: {
    profile: ValidatedProfile['perfil'];
    score: number;
  }) => (
    <div className="flex items-center space-x-4 p-4 border rounded-lg bg-gray-50">
      <img
        ...
        onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) =>
          (e.currentTarget.src = '/img/placeholder_profile.png')
        }
      />
    ...
    <div className="container mx-auto py-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="font-semibold text-lg">
                {(resultado.entity as Entidade).razao_social ||
                  (resultado.entity as Entidade).nome}
              </p>
            </div>
            <div>
              <p className="text-sm font-medium text-gray-500">Tipo</p>
              <p className="font-semibold capitalize">
                {(resultado.entity as Entidade).tipo}
              </p>
            </div>
      ...
      {resultado.validated_profile && (
        <Card className="mb-6 bg-green-50 border-green-200 shadow">
          ...
          <CardContent>
            <ValidatedProfileDisplay
              profile={resultado.validated_profile.perfil}
              score={resultado.validated_profile.score}
            />
          </CardContent>
        </Card>
      )}
      ...
      <Card className="mb-6 bg-blue-50 border-blue-200 shadow">
        <CardHeader>
          <CardTitle className="text-blue-800 flex items-center">
            <Save className="mr-2 h-5 w-5" /> Confirmar/Inserir Instagram para a
            Entidade
          </CardTitle>
        </CardHeader>
        ...
        <CardContent>
          {resultado.candidates.length === 0 ? (
            <p className="text-sm text-gray-500 text-center py-4">
              Nenhum candidato sugerido encontrado.
            </p>
          ) : (
            <Table>
              <TableHeader>
                  ...
                  <TableRow key={candidato.pk}>
                    ...
                    <TableCell className="text-right">
                      <Button
                        ...
                        variant="ghost"
                        onClick={() =>
                          setUsernameParaConfirmar(candidato.username)
                        }
